...
Fortran, with
[multithreading](../C14-SharedMemory/multicore.html#_programming_multicore_systems)
and [message
passing](distrmem.html#_distributed_memory_systems_message_passing_and_mpi)
enabled with libraries such as [POSIX
threads](../C14-SharedMemory/posix.html#_hello_threading_writing_your_first_multithreaded_program),
...
Figure 1 highlights the key differences in software utilized by
the HDA and HPC communities. Note that both communities use similar
cluster hardware that follows a [distributed
memory](distrmem.html#_distributed_memory_systems_message_passing_and_mpi)
...
processors and frequently a GPU. The cluster
...
#### Understanding Map and Reduce Operations 
...
`scalarMultiply` function in Table 1 yields a combined sum
of 50.
...
For simplicity, in Figure 2 we illustrate how MapReduce
...
Figure 2 gives an overview of this process. Prior to
...
(*key*,list(*value*)) pairs. In Figure 2, for example,
...
reduce tasks. In Figure 2, *R* = 8. The framework then
